...
Then(/^I should see results as "([^"]*)"$/) do |result|
...
#find_elements(id: "contentPanel").find_element(xpath: "//android.widget.TextView[@text='#{value}']").click
#end

Then(/^I select "([^"]*)" from menu$/) do |value|
  find_elements(id: "category_list_drawer").find_element(xpath: "//android.widget.TextView[@text='#{value}']").click
end
...
Then(/^I should see text "([^"]*)"$/) do |result|
  value = find_element(id: "text_info_history").text
  if value != result
    fail("expected value is #{result}, actual value is #{value}")
  end
end
